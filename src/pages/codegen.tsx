...
const CodeGen: React.FC = () => {
  ...
  const fixTheCode = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(error);
      alert('An error occurred while fixing the code');
    } finally {
      setLoading(false);
    }
  };

  const parseGitHubUrl = (url: string): Record<string, string> => {
    const regex =
  ...
  const getCode = async (): Promise<void> => {
    try {
      const githubInfo = parseGitHubUrl(filePath);
  ...
  const submitToGitHub = async (): Promise<void> => {
    try {
      ...
      alert('An error occurred while submitting to GitHub');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        >
          ...
          >
            ...
            <Input
              ...
              width={'100%'}
              defaultValue={filePath}
              placeholder='Enter the GitHub URL to a file you want to analyze'
            />
            <Flex
